...
export class Master_Timetable {
  constructor(num1, num2) {
    this.timetable = new Array(num1)
      .fill(null)
      .map(() => new Array(num2).fill(null));
  }
  ...
  displayTimetable() {
    for (let day = 0; day < this.timetable.length; day++) {
      for (
        let timeSlot = 0;
        timeSlot < this.timetable[day].length;
        timeSlot++
      ) {
...
//Slots where  we need not to edit or slots where we need not to schduled
export class Non_editable_cells {
